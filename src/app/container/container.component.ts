...
export class ContainerComponent implements AfterViewInit, OnInit {
  ...
  ngAfterViewInit(): void {
    let projectNavList: [number, string][] = this.projects.map((a) => [
      a.id,
      a.name.replace(/\s/g, ''),
    ]);
    this.selectedProjectFragment = window.location.hash.substring(1);
    if (
      this.selectedProjectFragment != null &&
      projectNavList.map((a) => a[1]).includes(this.selectedProjectFragment)
    ) {
      let selectedProjectNav: [number, string][] = projectNavList.filter(
        (a) => a[1] == this.selectedProjectFragment
      );
  ...
  openProjectbyFragment(selectedProjectNav: [number, string][]) {
    ...
    setTimeout(() => {
      this.showDetails(
        this.projects.filter((a) => {
          return a.id == selectedProjectNav[0][0];
        })[0]
      );
    }, 900);
  }
  ...
  projects = [
    {
      ...
      description: 'Search and retreive popular cocktail recipes',
      image: 'cocktail.jpg',
    },
    {
      id: 102,
      name: 'Encryptor',
      description: 'Encrypts text and decrypts ciphertext using AES protocol.',
      image: 'encryptor_cover.jpg',
    },
    {
      id: 103,
      name: 'QR Code Generator',
      description: 'Generates QR Code based on text input.',
