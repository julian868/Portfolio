...
export class CocktailRecipesComponent {
  ...
  async searchCocktails() {
    if (this.searchInput != '') {
      this.http
        .get<any>(
          `https://www.thecocktaildb.com/api/json/v1/1/search.php?s=${this.searchInput}`
        )
        .subscribe((res) => {
  ...
  async viewRecipe(cocktail: any) {
    ...
    for (let i = 1; i <= 15; i++) {
      ...
      this.measurements.set(
        cocktail.cocktail[ingredientKey],
        cocktail.cocktail[measureKey]
      );
      this.http
        .get(
          `https://www.thecocktaildb.com/images/ingredients/${cocktail.cocktail[ingredientKey]}-Small.png`,
          { responseType: 'blob', observe: 'body' }
        )
        .pipe(switchMap((blob) => this.convertBlobToBase64(blob)))
        .subscribe((base64ImageUrl) =>
          this.ingredientImages.set(
            cocktail.cocktail[ingredientKey],
            base64ImageUrl
          )
        );
